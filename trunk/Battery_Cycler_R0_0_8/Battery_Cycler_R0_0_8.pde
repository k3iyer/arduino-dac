...
void serial_monitor()
{
   ...
   //NOTE: this is currently just forwarding the raw data from the serial buffer but we still need to define a format for sending data how to decode it and then pick out the request ID, 
   //current setup is just for testing.
   if (Serial.available() > 0) 
     {
        // read the incoming byte:
        int request = Serial.read(); 
    
        if (request >= 49 && request <= 52)
          {
